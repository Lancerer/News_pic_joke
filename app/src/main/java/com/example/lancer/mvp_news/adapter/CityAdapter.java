...
public class CityAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> implements View.OnClickListener {
    public List<CityBean.NewslistBean> citylist ;
    Context mContext;
    BitmapUtils bitmapUtils;
    private static final int TYPE_ITEM = 0;  //普通Item View
    private static final int TYPE_FOOTER = 1;  //顶部FootView

    public CityAdapter(Context context, List<CityBean.NewslistBean> list) {
        citylist = list;
        mContext = context;
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        if (holder instanceof ViewHolder) {
            ...
            String picUrl = citylist.get(position).getPicUrl();
            String replace = picUrl.replace("\\", "");
            bitmapUtils.display(viewHolder.ivnew, replace);
            viewHolder.tvtitle.setText(citylist.get(position).getTitle());
            viewHolder.tvtime.setText(citylist.get(position).getCtime());
